-- Lesson 01 Example: SELECT, WHERE, ORDER BY, LIMIT
...
-- Main demonstration: filter customers, sort the result set, and cap the output.
SELECT id, name, country, signup_date
FROM customers
WHERE country IN ('EG', 'US')
  AND signup_date >= DATE '2024-02-01'
ORDER BY country ASC, signup_date DESC, name ASC
LIMIT 4;

-- Helpful variation: find the newest non-cancelled orders.
