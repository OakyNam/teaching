-- Lesson 04 Example: subqueries and CTEs
...
-- Subquery in WHERE: customers whose spend is above the average customer spend.
SELECT name
FROM customers
WHERE id IN (
    SELECT customer_id
...
-- Subquery in SELECT: show each order with its item count.
SELECT
    o.id,
    o.status,
...
-- CTE: compute customer spend once, then reuse it.
