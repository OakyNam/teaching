-- Lesson 05 Example: data modeling and normalization
...
-- Step 1: create normalized tables for the same e-commerce data.
...
-- Step 2: load each entity once so repeating values are not duplicated.
...
-- Main demonstration: normalized tables make reporting easier and avoid repeated customer/product text.
SELECT c.name, COUNT(DISTINCT o.id) AS total_orders, SUM(oi.quantity * oi.unit_price) AS revenue
FROM customers c
