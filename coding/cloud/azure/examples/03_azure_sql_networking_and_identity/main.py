"""Build an Azure SQL connection target with a SQLite fallback."""
...
def build_connection_string() -> str:
    ...
    sqlite_path = Path(__file__).with_name('azure_sql_demo.sqlite3')
...
def run_local_demo(sqlite_url: str) -> None:
    ...
    with sqlite3.connect(sqlite_path) as conn:
        ...
        conn.execute('INSERT INTO checks (name, passed) VALUES (?, ?)', ('private-endpoint-ready', 1))
...
def main() -> None:
    ...
    if connection_string.startswith('sqlite:///'):
        print('No Azure SQL env vars detected; running the SQLite fallback demo.')
