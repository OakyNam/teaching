...
def make_request(base_url: str, method: str, path: str, *, body: dict[str, Any] | None = None) -> None:
    token = os.getenv('BOOKS_API_TOKEN', 'replace-with-dev-token')
    headers = {'Accept': 'application/json', 'Authorization': f'******'}
...
def main() -> None:
    ...
    thread.start()
    base_url = os.getenv('BOOKS_API_BASE_URL', f'http://127.0.0.1:{server.server_port}')
