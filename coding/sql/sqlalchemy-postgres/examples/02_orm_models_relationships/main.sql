...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        order = Order(status="paid", customer=customer, items=[OrderItem(product=mouse, quantity=2), OrderItem(product=keyboard, quantity=1)])
        session.add_all([customer, mouse, keyboard, order])
        session.commit()

        stmt = select(Customer).where(Customer.email == "alice@example.com")
        loaded_customer = session.scalars(stmt).one()
        print(loaded_customer.name, "has", len(loaded_customer.orders), "order(s)")
        print("Validation passed:", loaded_customer.orders[0].items[0].product.name == "Wireless Mouse")
