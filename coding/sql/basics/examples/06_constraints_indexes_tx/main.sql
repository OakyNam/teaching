-- Lesson 06 Example: constraints, indexes, and transactions
...
-- Additional performance helpers for common access patterns.
CREATE INDEX idx_orders_customer_date ON orders (customer_id, order_date DESC);
CREATE INDEX idx_products_category_price ON products (category, price DESC);

-- Main demonstration: create an order inside a transaction.
BEGIN;
    INSERT INTO orders (id, customer_id, order_date, status, shipping_country)
    VALUES (1007, 2, DATE '2024-04-12', 'paid', 'EG');

    INSERT INTO order_items (order_id, product_id, quantity, unit_price)
    VALUES (1007, 7, 1, 220.00);
COMMIT;

SELECT o.id, c.name, o.status, SUM(oi.quantity * oi.unit_price) AS order_total
FROM orders o
...
FROM orders
WHERE id = 1001;
-- Expected status after rollback: paid
