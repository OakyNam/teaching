-- Lesson 07 Example: SQL injection protection
...
-- Unsafe pattern to avoid: concatenating user input directly into SQL text.

-- Safe pattern: prepare once, bind values later.
PREPARE customer_by_email(text) AS
SELECT id, name, email
FROM customers
WHERE email = $1;

EXECUTE customer_by_email('bob@example.com');
EXECUTE customer_by_email('bob@example.com'' OR ''1''=''1');
-- The second EXECUTE returns zero rows because the malicious text is treated as data.

PREPARE orders_by_status_and_country(text, text) AS
...
EXECUTE orders_by_status_and_country('shipped', 'EG');
