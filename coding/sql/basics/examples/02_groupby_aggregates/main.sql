-- Lesson 02 Example: GROUP BY, HAVING, and aggregates
...
-- Main demonstration: summarize customer revenue across paid/fulfilled orders.
SELECT
    c.id AS customer_id,
    c.name,
...
HAVING SUM(oi.quantity * oi.unit_price) > 400
ORDER BY total_revenue DESC;

-- Category summary variation.
SELECT
    p.category,
    COUNT(*) AS line_count,
...
GROUP BY p.category
ORDER BY units_sold DESC, p.category ASC;
